...
- Cafe24 서버에서 readiness 정상 응답 판정 오류를 바로잡고 adoption을 마친다.
...
- Nuxt의 정상 `status: ok` 응답을 설치기가 거부하던 readiness 판정 오류를 수정했다.
...
- v1.2.6 adoption은 Nuxt `/ready`의 정상 `status: ok`를 설치기가 `ready`만 기대해 200 응답 뒤 중단된다.
...
- v1.2.7 통합 asset을 게시하고 실패했던 Cafe24 adoption을 재실행한다.
