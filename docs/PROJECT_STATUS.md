...
- Sensta iOS `82e3b59`에 이메일 로그인·Keychain 세션 복원·401 토큰 갱신·로그아웃과 내 공개 프로필
  ...
  서명 앱은 iPhone 17에 설치했고 기기 잠금으로 자동 실행은 되지 않았다. GOAPI·Android 변경이나
  서버 교체는 없으며 다음은 본인 댓글 수정·삭제다.
...
4. Sensta iOS 상세의 좋아요·취소와 피드 수치 공유를 실기기에서 확인한다. 다음은 댓글 작성·답글과
   댓글 좋아요다. 인증 토큰 갱신과 익명 사진 감상은 구현된 공용 경로를 유지한다. Google·Apple
   로그인 설정은 해당 작업 때 안내한다. GOAPI 변경이 필요하면 로컬 Colima와 공식
