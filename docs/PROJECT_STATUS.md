...
- 게시글·댓글 다중 리액션 피드백 반영(2026-09-24): `.ai/FEEDBACK.md`의 P0·P1을 두 저장소에 반영했다.
  GOAPI는 설치·이행 경로와 읽기 SQL 회귀를 실제 MariaDB 통합 테스트로 고정했고(새 설치/레거시 이행
  재실행, 중복 정리, `liked=(reaction_type=1)` 불변식, 0→종류→교체→취소 전이, 동시 요청 1행),
  JSON 계약을 PLAN대로 상위 레벨 `reactions`/`myReaction`으로 일치시키고 취소(null)·빈 문자열 거부·
  첫 like INSERT 알림 누락·삭제/비밀/차단 권한 검사를 고쳤다. 홈·스튜디오 응답도 묶음 집계로 채운다.
  웹은 플랫 계약, 목록/홈/검색/스튜디오 종류별 요약 표시, 비로그인 선택기의 로그인 복귀 흐름,
  구 응답 `like/liked` 이관, 미사용 import 제거를 반영했다. `go test ./...`·`go vet`·실DB 통합 테스트,
  웹 lint(0 error)·typecheck·115+7 테스트·build 통과. 커밋·push 후 제품 소유자 화면 QA가 남아 있다.
  운영 배포는 여전히 보류(공식 `build-ubuntu22.sh` 바이너리 준비·`goapi install` 검증 전까지).
