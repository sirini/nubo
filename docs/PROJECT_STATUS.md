...
- Market 제작자·운영자 게시 흐름을 운영 검수까지 마쳤다. 다음 작업은 nubohub.org 로그인 사용자가 남기는 최소 스킨 리뷰·별점이다.
...
1. 스킨 리뷰·별점의 NUBO 로그인 연동과 계정당 스킨별 1개 데이터 계약을 설계한다.
2. 작성·수정 UI, 목록·상세 표시와 최소 운영자 관리 기능을 구현하고 권한·중복·CSRF 회귀 테스트를 추가한다.
3. nubohub.org에서 실제 사용자 흐름을 검수하고 필요한 수정·릴리스를 마친 뒤 장기 로드맵을 다시 리뷰한다.
