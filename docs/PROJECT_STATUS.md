...
- 게시글·댓글 다중 리액션 2차 리뷰(2026-09-24): 설치 이행·핵심 조회·취소 전이는 개선되었고 워커가
  격리 MariaDB 신규/기존 설치·동시성 검증을 기록했다. 재검토에서 `reaction` 필드 누락을 취소로
  받아들이는 API 계약 오류, 게시판 열람 레벨/포인트를 우회하는 쓰기 권한, 삭제 댓글 리액션 허용을
  확인했다. 고급 블로그/갤러리 상세·댓글, 고급 홈의 선택기와 일부 목록 요약도 미완료다. 로컬
  `.ai/FEEDBACK.md`에 수정·재검증 기준을 기록했다. 이번 리뷰의 GOAPI test/vet, 웹 lint/typecheck는
  통과했다. 코드 완료 선언과 운영 배포는 재수정·재검토 이후로 보류한다. `.ai`는 Git 추적에서 제거했다.
