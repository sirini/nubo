...
- 실기기 목록 피드백은 iOS `afb2ae4`에서 반영했다. 모든 작품 미리보기를 정사각형으로 고정하고 사진을
  경계 안에서 잘라 동일한 라운딩을 적용했으며, 짧게 끊기던 기본 구분선 대신 여백이 같은 독립 카드로
  정리했다. 전체 단위 111개와 일반·큰 글자 스튜디오 UI 2개, Release build·정적 분석을 통과하고 수정
  앱을 iPhone 17에 다시 설치했다.
...
- iOS `afb2ae4`에서 스튜디오 목록을 1:1 미리보기와 동일한 12pt 라운딩으로 통일하고 이미지 넘침을
  차단했다. 기본 행 구분선은 제거하고 각 작품을 독립 카드로 분리했으며 일반·큰 글자 캡처와 관련 UI
  테스트 2개로 확인했다.
