...
- 어제까지의 Market·릴리스 작업을 리뷰했다. 결합된 기본 게시판 스킨을 게시판·블로그·갤러리 3개로 분리하고, 제작자가 복사해 확장하기 쉬운 provider 설명과 Market 패키지를 준비한다.
...
- 스킨 로더는 선택한 스킨에 종류별 엔트리가 없을 때 `Default*`를 먼저 사용한다. 기본 블로그·갤러리를 별도 패키지로 옮길 때 기존 `nubo-basic-board` 설정이 일반 게시판 UI로 바뀌지 않도록 종류별 호환 fallback과 회귀 테스트가 필요하다.
- 게시판 스킨의 공통 목록·글보기·글쓰기 컴포넌트가 `nubo-basic-board` 안에 있어 다른 스타일의 공식 스킨도 해당 폴더에 결합된다. 공용 UI 기반과 스타일별 엔트리의 경계를 분리하고 provider 계약을 스킨 안에서 설명해야 한다.
...
- 기본 게시판·블로그·갤러리 스킨 분리와 Market 등록을 완료한 뒤, 첫 실제 제작자 계정과 테스트 key를 이용한 운영 절차 리허설은 다음 작업 후보 목록 뒤로 미룬다. 결제·커미션은 계속 보류한다.
