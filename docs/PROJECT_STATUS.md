...
- 이미지 설명 모델 전환(2026-09-25): GOAPI `672e9eb` 기본값과 웹 예시 설정·소급 처리 스크립트를
  `gpt-6-luna`로 갱신하고 소급 처리 비용 추정을 공식 입력 $0.10/출력 $0.50(100만 토큰당)으로
  조정했다. 기존 Chat Completions 이미지 입력 `detail=low`, `reasoning_effort=none`, 출력 상한
  220토큰을 유지하고 `temperature`는 보내지 않는다. OpenAI 공식 모델·마이그레이션 문서에서
  호환성을 확인했고 GOAPI 전체 test/vet, 소급 처리 테스트 7개를 통과했다. 로컬 비공개 `.env`의
  명시적 모델명도 갱신했으며 기능 opt-in은 false로 유지했다. 실제 API 호출·운영 설정 반영은
  아직 하지 않았다.
