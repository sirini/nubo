...
- Sensta iOS `82e3b59`에 이메일 로그인·Keychain 세션 복원·401 토큰 갱신·로그아웃과 내 공개 프로필
  ...
  11개와 Debug/Release 빌드를 통과하고 iPhone 17에 설치·실행했다. 실제 계정 QA와 인증 기반
  좋아요·댓글 참여 연결이 다음 단계다.
...
4. Sensta iOS에서 기존 이메일 로그인 → 내 공개 프로필 → 앱 재실행 시 세션 복원 → 로그아웃을
   실기기에서 확인한다. 다음은 좋아요·댓글 참여와 개인별 조회 상태·토큰 갱신 연결이다. Google·Apple
