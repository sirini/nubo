...
대상: NUBO `main`의 다중 리액션 웹, GOAPI `80be83f` 이상. Ubuntu 22.04 amd64 소스 설치 기준.
...
- 로그인 사용자 A로 게시글과 댓글에 `좋아요 → 최고 → 아차 → 글쎄요 → 취소`를 적용하고 목록·상세 새로고침 값이 일치하는지 확인한다. 현재 종류를 다시 선택했을 때도 취소되어야 한다.
...
- 기존 iOS/Android의 좋아요·취소가 다른 종류의 리액션을 지우지 않고 `like` 수만 바꾸는지 확인한다.
