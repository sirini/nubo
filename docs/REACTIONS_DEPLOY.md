...
대상: NUBO `main`의 다중 리액션·다층 댓글 웹, GOAPI `e7aa0b0` 이상(확장 여섯 종류 리액션과 다층 댓글 포함; `74123ec`은 리액션만, `80be83f`는 기존 네 종류만). Ubuntu 22.04 amd64 소스 설치 기준.
...
- 댓글에 루트 → 답글 → 답글의 답글(3단계 이상)을 남기고 들여쓰기·“OO님께 답글” 표시·접기/펼치기가
  맞는지, 새로고침 후에도 트리가 유지되는지 확인한다. 답글이 달린 중간 댓글을 삭제하면 `(deleted)`로
  남고 자손은 유지되어야 한다.
