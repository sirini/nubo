export const useVerifyCodeTemplate = () => {
  const config = useRuntimeConfig()

  // 아래에서 인증 코드 발송용 메일 본문을 직접 수정할 수 있습니다
  // {{Code}} 는 인증 코드로 치환될 부분입니다
  const template = `
<!DOCTYPE html>
<html>
<head>
          ...
          <tr>
            <td style="padding: 50px 40px; text-align: center;">
              ...
              <p style="margin: 0 0 40px 0; font-size: 16px; color: #4a4a4a; line-height: 1.6;">
                아래의 인증 코드를 ${config.public.title}에 입력하여<br>이메일 인증을 완료해 주세요.
              </p>

              <div style="background-color: #1e2128; padding: 30px; border-radius: 8px; margin-bottom: 40px;">
                <span style="font-family: 'Courier New', Courier, monospace; font-size: 36px; font-weight: bold; color: #ffffff; letter-spacing: 10px;">
                  {{Code}}
                </span>
              </div>

              <p style="margin: 0; font-size: 14px; color: #888888;">
                본인이 계정을 생성하지 않았다면 이 메일을 무시하셔도 됩니다.
              </p>
            </td>
          </tr>
        </table>
