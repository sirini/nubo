export const useApi = () => {
  ...
  const reqGet = async <T>(url: string, query: Record<string, any>) => {
    return await $api<T>(url, { method: "GET", query })
  }

  // Composables에서 사용할 클라이언트용 POST
  const reqPost = async <T>(url: string, body: Record<string, any> | BodyInit | FormData) => {
  ...
  const reqDelete = async <T>(url: string, query: Record<string, any>) => {
